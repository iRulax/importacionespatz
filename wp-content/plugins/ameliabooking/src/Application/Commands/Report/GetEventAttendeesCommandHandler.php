    ...
    public function handle(GetEventAttendeesCommand $command)
    {
        ...
        foreach ($event->getBookings()->getItems() as $index => $booking) {
            ...
            if (in_array('paymentAmount', $fields, true)) {
                ...
                $amount  = $payment ? $payment->getAmount()->getValue() : 0;
            ...
            if (in_array('paymentStatus', $params['fields'], true)) {
                ...
                $row[BackendStrings::getCommonStrings()['payment_status']] .= ($status === 'partiallyPaid' ? BackendStrings::getCommonStrings()['partially_paid'] : BackendStrings::getCommonStrings()[$status]) . $delimiterSeparate;
            }
            ...
            if (in_array('paymentMethod', $params['fields'], true)) {
                ...
                $row[BackendStrings::getCommonStrings()['payment_method']] .= (!$method || $method === 'onSite' ? BackendStrings::getCommonStrings()['on_site'] : BackendStrings::getSettingsStrings()[$method]) . $delimiterSeparate;
            }

...
class AppointmentReservationService extends AbstractReservationService
{
    ...
    public function bookSingle($reservation, $appointmentData, $inspectTimeSlot, $save)
    {
        ...
        if ($inspectTimeSlot) {
            ...
            if (!$isPackageBooking && (!isset($appointmentData['isBackendOrCabinet']) || !$appointmentData['isBackendOrCabinet']) && $this->checkLimitsPerCustomer($service, $appointmentData['bookings'][0]['customerId'], $appointment->getBookingStart()->getValue())) {
                throw new BookingUnavailableException('');
            }
